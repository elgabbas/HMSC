                        ...
                        if (predictMean || predictMeanField) {
                           if (!is.null(rL$s)) {
                              ...
                              if (is(s1, "Spatial")) {
                                 ...
                                 dim = NCOL(s1)
                                 D11 = as.matrix(dist(s1))
                                 D12 = sqrt(Reduce("+", Map(function(i) outer(s1[, i], s2[, i], "-")^2, seq_len(dim))))
                              }
                           } else {
                              D11 = rL$distMat[units, units, drop = FALSE]
                              D12 = rL$distMat[units, unitsPred[indNew], drop = FALSE]
                           ...
                           switch(rL$spatialMethod, Full = {
                              unitsAll = c(units, unitsPred[indNew])
                              if (!is.null(rL$s)) {
                                 s = rL$s[unitsAll, , drop = FALSE]
                                 if (is(s, "Spatial")) D <- spDists(s) else D = as.matrix(dist(s))
                              } else {
                                 D = rL$distMat[unitsAll, unitsAll]
                              }
                              for (h in 1:nf) {
                                 if (alphapw[alpha[h], 1] > 0) {
                                    ...
                                    W = K22 - crossprod(K12, solve(K11, K12))
                                    L = try(t(chol(W)))
                                    if (inherits(L, "try-error")) etaPred[indNew, h] <- m else etaPred[indNew, h] = m +
                                       L %*% rnorm(nn)
                                 } else {
                                    etaPred[indNew, h] = rnorm(nn)
                                 }
                              ...
                              if (is.null(rL$s)) {
                                 ...
                                 if (is(sOld, "Spatial")) {
                                    ...
                                    indNN <- matrix(0, nn, nnabo)
                                    for (i in seq_len(nn)) {
                                       indNN[i, ] <- order(spDists(sOld, sNew[i, , drop = FALSE]))[seq_len(nnabo)]
                                    }
                                 } else {
                                    sNew <- as.matrix(sNew)
                                    indNN = knnx.index(sOld, sNew, k = rL$nNeighbours)
                                 ...
                                 for (i in 1:nn) {
                                    ind = indNN[i, ]
                                    indices[[i]] = rbind(i * rep(1, length(ind)),
                                                         ind)
                                    if (is(sOld, "Spatial")) {
                                       dist12[, i] <- spDists(sOld[ind, , drop = FALSE], sNew[i, ])
                                       dist11[, , i] = spDists(sOld[ind, , drop = FALSE])
                                       ...
                                       das <- 0
                                       for (dim in seq_len(rL$sDim)) das <- das +
                                             (sOld[ind, dim] - sNew[i, dim])^2
                                       dist12[, i] <- sqrt(das)
                                       dist11[, , i] <- as.matrix(dist(sOld[ind,
                                       ]))
                                    }
                              ...
                              for (h in 1:nf) {
                                 if (alphapw[alpha[h], 1] > 0) {
                                    K12 = exp(-dist12/alphapw[alpha[h], 1])
                                    ind1 = t(matrix(unlist(indices), nrow = 2))[,
                                                                                2]
                                    ind2 = t(matrix(unlist(indices), nrow = 2))[,
                                                                                1]
                                    K11 = exp(-dist11/alphapw[alpha[h], 1])
                                    K21iK11 = matrix(NA, ncol = nn, nrow = rL$nNeighbours)
                                    for (i in 1:nn) {
                                    ...
                                    Fmat = 1 - colSums(K21iK11 * K12)
                                    m = B %*% eta[, h]
                                    etaPred[indNew, h] = as.numeric(m + sqrt(Fmat) *
                                                                       rnorm(nn))
                                 } else {
                                    etaPred[indNew, h] = rnorm(nn)
                                 }
                              ...
                              if (is(s, "Spatial")) {
                                 ...
                                 dim = NCOL(s)
                                 dss = as.matrix(dist(sKnot))
                                 das = sqrt(Reduce("+", Map(function(i) outer(s[, i], sKnot[, i], "-")^2, seq_len(dim))))
                              }
                              dns = das[np + (1:nn), ]
                              dnsOld = das[1:np, ]
                              for (h in 1:nf) {
                                 ...
                                 if (alphapw[ag, 1] > 0) {
                                    ...
                                    WnsiWss = Wns %*% iWss
                                    dDn = 1 - rowSums(WnsiWss * Wns)
                                    if (any(dDn < 0)) dDn[dDn < 0] <- 0
                                    D = W12 %*% iWss %*% t(W12)
                                    dD = 1 - diag(D)
                                    idD = 1/dD
                                    tmp0 = matrix(rep(idD, NROW(sKnot)),
                                                  ncol = NROW(sKnot))
                                    idDW12 = tmp0 * W12
                                    FMat = Wss + t(W12) %*% idDW12
                                    iF = solve(FMat)
